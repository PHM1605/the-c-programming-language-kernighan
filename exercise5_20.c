// expand dcl.c to handle declarations with function argument types, qualifiers like const
// e.g. handle int (*f)(const char *, int)
#include <stdio.h>
#include <string.h>
#include <ctype.h>
...
int gettoken() {
    ...
    if (c == '(') {
        ...
        } else if (isalpha(c)) { // (const...)
            *p++ = '(';
            for (*p++=c; (*p++=getch())!=')';);
            *p = '\0'; // token = (const char*, int)
            return tokentype = PARAMS;
        } else { // (*f) in function name
            ungetch(c);
            return tokentype = '(';
        }
    } else if (c=='[') {
